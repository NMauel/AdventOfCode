...
public class Day19 : IPuzzleDay
{
    ...
    private static IEnumerable<Scanner> ParseToScanners(IEnumerable<string> input)
    {
        ...
        foreach (var line in input.Where(l => !string.IsNullOrWhiteSpace(l)))
        {
            if (line.StartsWith("---"))
            {
                ...
                {
                    yield return new(currentScannerID, beaconCoordinates.ToArray());
            ...
            else
            {
                ...
                beaconCoordinates.Add(new(coords[0], coords[1], coords[2]));
            }
        }
        yield return new(currentScannerID, beaconCoordinates.ToArray());
    }
    ...
    private class Scanner
    {
        ...
        public Scanner(int scannerID, Coordinate[] beaconCoordinates)
        {
            ...
            for (var a = 0; a < beacons.Count - 1; a++)
            for (var b = a + 1; b < beacons.Count; b++)
            {
                distances.Add(new(beacons[a], beacons[b]));
            }
        }
